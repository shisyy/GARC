...
from splart.jsa_ect import (canonical_object_scalar, feature_crossfit_jsa_ect,
                            domain_receipt_passes, global_receipt_passes, global_row_identity, jsa_ect_ablation,
...
FROZEN_CONFIG_SHA256 = "a89543b1cf954cfc2de191fb50dda265ca8182dcac9e1a7d14a8faed90f853b2"
PREDECESSOR_CONFIG_SHA256 = "779874c52dc3c64b3e6606ad11c4b0a2e3d07fe5b814054b7d2ce8a0ff852be5"
...
def verify_frozen_source_payloads(args, config: dict, predecessor: dict) -> None:
    ...
    for key in ("dino_checkpoint_sha256","render_log_sha256","object_list_hashes","articraft_inputs_sha256","articraft_manifest_sha256","njc_inputs_sha256","njc_manifest_sha256","render_feature_payload_sha256"):
    ...
    for path, expected in zip(caches,source["render_feature_payload_sha256"]):
        actual={"inputs":sha256_file(path/"inputs.pt"),"manifest":sha256_file(path/"manifest.json"),"render_bank":sha256_file(path/"neutral_render_bank.pt")}
...
def run_cell(rows, train, held, train_field, held_field, train_z, held_z, cfg, context, kind):
    ...
    for d in first[2]["domains"]:
        left, right = first[2]["domains"][d]["crossfit"], second[2]["domains"][d]["crossfit"]
        if left is right or left["folds"] is right["folds"] or any(a is b for a,b in zip(left["folds"],right["folds"])):
            raise RuntimeError("nested receipt alias")
    ...
    audit = {"repeat_bit_identical": torch.equal(first[0], second[0]) and torch.equal(first[1], second[1]) and first[2] == second[2],
             "row_order_invariant": same(_remap(rows, canonical_train, first[0]), _remap(rows, canonical_train, reverse[0])) and same(_remap(rows, canonical_held, first[1]), _remap(rows, canonical_held, reverse[1])) and first[2] == reverse[2]}
...
def verify_provenance_binding(result: dict, receipt: dict) -> bool:
    receipt_keys={"schema","config_sha256","source_config_sha256","predecessor_config_sha256","feasibility_sha256","decision","code_sha256","feature_provenance_sha256","source_labels_opened","source_labels_hashed","source_scores_computed","training_started","remote_execution_started","execution_environment","execution_phase","box_labels_read","protected_splits_read"}
    result_keys={"schema","config_sha256","source_config_sha256","predecessor_config_sha256","feature_provenance","feature_provenance_sha256","jsa_ect","independent_recomputation_sha256","runtime_audits","global_receipt_pass","domain_receipt_pass","cell_pass","all_pass","object_list_hashes","global_row_identity_sha256","code_files_sha256","code_sha256","source_labels_opened","source_labels_hashed","source_scores_computed","training_started","remote_execution_started","execution_environment","execution_phase","box_labels_read","protected_splits_read"}
    try:
        if set(receipt)!=receipt_keys or set(result)!=result_keys: return False
        if receipt["schema"]!="splart-jsa-ect-feasibility-receipt/v1" or result["schema"]!="splart-jsa-ect-feasibility/v1": return False
        ...
        if not isinstance(receipt["feasibility_sha256"],str) or len(receipt["feasibility_sha256"])!=64: return False
        ...
        return (all(result[k] is False and receipt[k] is False for k in ("source_labels_opened","source_labels_hashed","source_scores_computed","training_started"))
                ...
                and result["box_labels_read"]==receipt["box_labels_read"]==[] and result["protected_splits_read"]==receipt["protected_splits_read"]==[])
...
def main() -> None:
    ...
    parser.add_argument("--source-config", type=Path, required=True); parser.add_argument("--predecessor-config",type=Path,required=True); parser.add_argument("--config", type=Path, required=True); parser.add_argument("--output", type=Path, required=True)
    ...
    if config["source_config_sha256"]!=SOURCE_CONFIG_SHA256 or config["predecessor_rza_config_sha256"]!=PREDECESSOR_CONFIG_SHA256 or predecessor.get("schema")!="splart-rza-acwt-label-free-preregister/v1": raise RuntimeError("predecessor binding mismatch")
    ...
    verify_frozen_source_payloads(args,config,predecessor)
    ...
    observed={"articraft_train":hash_ids(sorted({rows[i]["object_group_id"] for i in train if rows[i]["domain"]=="articraft"})),"articraft_held":hash_ids(sorted({rows[i]["object_group_id"] for i in held if rows[i]["domain"]=="articraft"})),"njc_train":hash_ids(sorted({rows[i]["object_group_id"] for i in train if rows[i]["domain"]=="njc"})),"njc_held":hash_ids(sorted({rows[i]["object_group_id"] for i in held if rows[i]["domain"]=="njc"}))}
    ...
    if not bool(prep.mechanical_keep[-1]) or kept[-1]!=prep.mechanical_keep.numel()-1: raise RuntimeError("displacement coordinate is not final and kept")
    ...
    for name, cell in (("semantic", semantic), ("mechanical", mechanical)):
        ...
        cells[name] = {d: full[name] and domain_receipt_passes(cell[2],cell[3],cell[4],cfg,CONTEXTS[name],domains,d) and cell[5]["repeat_bit_identical"] and cell[5]["row_order_invariant"] and cell[5]["candidate_prediction_sha256"]==cell[5]["gate_recompute_prediction_sha256"] and (name != "mechanical" or d_exact) for d in domains}
    root = Path(__file__).resolve().parents[1]; code_files = [Path(__file__).resolve(), root/"src"/"splart"/"jsa_ect.py",root/"scripts"/"run_rza_acwt_label_free.py", root/"src"/"splart"/"rza_acwt.py",root/"scripts"/"run_rqlsot_feasibility.py",root/"src"/"splart"/"rqlsot.py",root/"src"/"splart"/"conditional_residual.py", root/"src"/"splart"/"smarc_source.py",root/"src"/"splart"/"smarc.py",root/"src"/"splart"/"frozen_visual.py", root/"scripts"/"run_smarc_source_gate.py",root/"scripts"/"build_smarc_articraft_shard.py",root/"scripts"/"build_smarc_njc_cache.py",root/"scripts"/"build_smarc_smoke.py"]
    ...
    identities=sorted(global_row_identity(r) for r in rows)
    result = {"schema": "splart-jsa-ect-feasibility/v1", "config_sha256": FROZEN_CONFIG_SHA256,"source_config_sha256":SOURCE_CONFIG_SHA256,"predecessor_config_sha256":PREDECESSOR_CONFIG_SHA256, "feature_provenance": provenance, "feature_provenance_sha256": feature_sha,
        "jsa_ect": {"semantic": semantic[2], "mechanical": mechanical[2]}, "runtime_audits": {"semantic": semantic[5], "mechanical": {**mechanical[5], "recipient_displacement_bitwise_unchanged": d_exact}},
        "independent_recomputation_sha256":{"semantic":semantic[4],"mechanical":mechanical[4]},"global_receipt_pass": full,"domain_receipt_pass":{name:{d:domain_receipt_passes(cell[2],cell[3],cell[4],cfg,CONTEXTS[name],domains,d) for d in domains} for name,cell in (("semantic",semantic),("mechanical",mechanical))}, "cell_pass": cells, "all_pass": all(full.values()) and all(v for part in cells.values() for v in part.values()),"object_list_hashes":observed,"global_row_identity_sha256":canonical_sha256(identities), "code_files_sha256": code, "code_sha256": code_sha,
    ...
    receipt = {"schema": "splart-jsa-ect-feasibility-receipt/v1", "config_sha256": FROZEN_CONFIG_SHA256,"source_config_sha256":SOURCE_CONFIG_SHA256,"predecessor_config_sha256":PREDECESSOR_CONFIG_SHA256, "feasibility_sha256": sha256_file(temporary/"feasibility.json"),
