...
from gsplat.distributed import all_gather_int32, all_gather_tensor_list, all_to_all_int32, all_to_all_tensor_list
...
def rasterization(
    ...
    motion_quats: Optional[Tensor] = None,  # [..., M, 4]
    motion_ids: Optional[Tensor] = None,  # [..., N]
    ...
    if motion_quats is not None:
        if motion_quats.ndim == 1:
            motion_quats = motion_quats[None]
        identities = torch.zeros(batch_dims + (1, 4), dtype=motion_quats.dtype, device=device)
        identities[..., 0] = 1
        motion_quats = torch.cat((motion_quats, identities), dim=-2)
        M = motion_quats.shape[-2]
        assert motion_quats.shape == batch_dims + (M, 4), motion_quats.shape
        motion_ids = torch.where(motion_ids < 0, motion_ids + M, motion_ids)
        assert motion_ids.shape == batch_dims + (N,), motion_ids.shape

    def reshape_view(C: int, world_view: torch.Tensor, N_world: list) -> torch.Tensor:
        view_list = list(
            map(lambda x: x.split(int(x.shape[0] / C), dim=0), world_view.split([C * N_i for N_i in N_world], dim=0))
        )
    ...
    if sh_degree is None:
        ...
        assert (
            ...
            colors.dim() == num_batch_dims + 4 and colors.shape[:-2] == batch_dims + (C, N) and colors.shape[-1] == 3
        ), colors.shape
        assert (sh_degree + 1) ** 2 <= colors.shape[-2], colors.shape
        if distributed:
            assert colors.dim() == num_batch_dims + 3, "Distributed mode only supports per-Gaussian colors."
    ...
    if sh_degree is None:
        ...
        if packed:
            dirs = (
                means.view(B, N, 3)[batch_ids, gaussian_ids] - campos.view(B, C, 3)[batch_ids, camera_ids]
            )  # [nnz, 3]
            if motion_quats is not None:
                dirs = quaternion_apply(
                    quaternion_invert(
                        motion_quats.view(B, M, 4)[batch_ids, motion_ids.view(B, N)[batch_ids, gaussian_ids]]
                    ),
                    dirs,
                )
            ...
            if colors.dim() == num_batch_dims + 3:
                ...
                shs = colors.view(B, C, N, -1, 3)[batch_ids, camera_ids, gaussian_ids]  # [nnz, K, 3]
            ...
            if motion_quats is not None:
                dirs = quaternion_apply(
                    quaternion_invert(
                        torch.gather(motion_quats, -2, motion_ids.unsqueeze(-1).expand(*motion_ids.shape, 4)).unsqueeze(
                            -3
                        )
                    ),
                    dirs,
    ...
    if distributed:
        if packed:
            ...
            offsets = torch.tensor([0] + N_world[:-1], device=gaussian_ids.device, dtype=gaussian_ids.dtype)
            offsets = torch.cumsum(offsets, dim=0)
            offsets = offsets.repeat_interleave(torch.stack(cnts))
    ...
    if render_mode in ["RGB+D", "RGB+ED"]:
        ...
        if backgrounds is not None:
            backgrounds = torch.cat([backgrounds, torch.zeros(batch_dims + (C, 1), device=backgrounds.device)], dim=-1)
