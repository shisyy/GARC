...
Conflicts: Pruned node8.11 showed per-domain rank-one anisotropic transport overfits NJC; this node shares only identifiable conditional structure across domains while preserving domain nuisance anchors and scales, and is pruned without changing rank, basis, folds or thresholds if any cell fails. [PRUNED]

**Insight**: Shared-axis energy conservation fixed tail inflation and yielded one fully passing semantic/Articraft cell, but finite-moment transport still leaves heterogeneous dependence: monotone norm trend on mechanical/Articraft and nonlinear/coplanar dependence on both NJC cells. The next mechanism must transform the conditional distribution, not add moment modes.
...
#### 8.13: Mechanism: Pooled Kernel Sliced Rosenblatt Transport (PKSRT) uses domain marginal quantile bijections, a canonical grouped-knot Articraft-stabilized shared kernel conditional CDF, a fixed identity-plus-DCT slice sequence, and a final per-domain raw-z QR anchor to form an invertible conditional distribution transport.
...
Conflicts: Nodes8.9-8.12 show linear, quadratic, per-domain rank-one and shared rank-one moment transports leave different dependence forms; this node changes to a complete conditional-distribution transport and is pruned without adding bases, directions, bandwidth choices or thresholds if it fails. Pre-implementation erratum: exact duplicate pooled rank knots are grouped by canonical key and their kernel weights summed, because the frozen 97/11 odd-sized grids necessarily share a=0; no jitter, parameter, search or threshold was added. [PRUNED]

**Insight**: [Pruned: PRUNE_LABEL_FREE: semantic/Articraft passed (Spearman=0.063798, dCor=0.354233, p99=1.066533); semantic/NJC failed Spearman=0.501140, dCor=0.509976, p99=1.756696; mechanical/Articraft failed p99=1.837604; mechanical/NJC failed dCor=0.591941 and p99=1.540105. Global receipts, independent replay, repeat/row-order, exact raw-z anchors and mechanical displacement passed; labels/scores/training/Box remained untouched.]

#### 8.14: Mechanism: Gauge-Equivariant Looped Primal-Dual Transformer (GLPDT) tokenizes each side's ordered multi-radius D2 profile and recurrently applies one pre-norm input-recalled Transformer block for four shared-weight loops to refine a positive endpoint-distance state and a terminal-violation state, with one shared side function and deep supervision at every loop.
...
Conflicts: Pruned [8.1]/[8.2] found no usable profile crossing and [8.13] inflated three tails; this node assumes no explicit crossing and instead performs contractive residual inference around the frozen D2 anchor, with fail-closed null and tail gates. [DONE] (score: 0.3276)
...
**Branch**: arbor/node-8-14-glpdt

#### 8.15: Mechanism: Anchor-Relative Log-Distance Primal-Dual Loop (ALD-PDL) replaces the additive correction cap with a cumulative log(target-distance / anchor-distance) primal state, while the recurrent dual state damps only increases in a source-train-defined log-energy and the same recalled pre-norm Transformer block remains tied for four loops.
Hypothesis: GLPDT already halves same-protocol source error and converges across loops, but its additive action space reaches only 6/12 validation endpoints; an anchor-relative log-distance action space preserves exact positivity and swap equivariance while allowing multiplicative contraction and expansion without a hand-tuned range cap.
Observable: On the unchanged 13/6 object-disjoint source B_dev, all 12 endpoint distances are representable, ALD-PDL lowers mean-side and worst-side NMAE versus node 8.14 and a parameter-matched one-loop control, exact swap error is <=1e-6, per-loop residual decreases, and target-relative p99 error does not regress versus frozen D2.
Conflicts: Validated [8.14] showed recurrent evidence accumulation works but its fixed additive cap leaves half the validation sides unreachable and yields prediction/anchor p99=32.8629; this node keeps the loop and changes the primal geometry rather than tuning the cap, seed, width, loop count, or split. [DONE] (score: 0.2721)

**Insight**: Anchor-relative log-distance removes the additive action-space floor: representability rose from 6/12 to 12/12, mean-side NMAE improved 16.94% versus node8.14 and 60.85% versus D2, worst-side NMAE fell to 0.324730, and target-relative p99 fell from D2 3.4413 to 1.1823. However the same trained parameters at one loop score 0.258452, beating the four-loop final state, and the last prediction residual rises from 0.01225 to 0.02027, proving the remaining problem is recurrent overshoot/non-contractive fixed-point dynamics rather than action-space reachability.

**Result**: PARTIAL_PASS_SOURCE: ALD-PDL beats node8.14 and D2 with exact swap 0 and 12/12 representability, but fails the LOOP benefit gate because parameter-matched one-loop inference is 5.01% better. report_sha256=90cceb566276a55e89b303e592f87a03f5e314255bd508e9510b4362629839d3 checkpoint_sha256=409143173061b710f65b1f2abaf57af08897162ff7dc65c69d2dedf1293915af; protected_splits_read=[].

**Branch**: arbor/node-8-15-ald-pdl

#### 8.16: Mechanism: Counterfactual Re-query Fixed-Point Loop (CR-FPL) turns each ALD-PDL log-distance estimate back into a differentiable query of the ordered D2 profile, injects the sampled local geometry and query residual as the next loop's primal-dual feedback token, and applies a shared convex fixed-point update with the same four tied pre-norm loops.
...
Conflicts: Pruned [8.1]/[8.2] showed no explicit signed-gap crossing, so CR-FPL does not threshold or search for a crossing; it uses the continuously sampled local profile only as iterative feedback, directly countering validated [8.15]'s static-input overshoot without changing seed, width, loop count, or split. [DONE] (score: 0.1982)
...
**Branch**: arbor/node-8-16-cr-fpl

#### 8.17: Mechanism: Swap-Equivariant Minimax Dual Coupler (SE-MDC) augments CR-FPL with a symmetric two-side attention step whose shared dual slack tokens estimate relative endpoint risk and jointly allocate each loop's convex log-distance update, trained by a smooth object-level worst-side saddle objective.
...
Conflicts: Validated [8.16] proves local re-query and fixed-point convergence but shows Articraft worst-side and both-domain p99 tail regressions; this node changes cross-side credit allocation rather than loop count, width, seed, thresholds, or data splits, and avoids the marginal-transport tail failures of pruned [8.11]-[8.13]. [DONE] (score: 0.2091)

**Insight**: SE-MDC validates exact swap-equivariant coupled recurrent mechanics and strong tail suppression, but does not improve the primary mean on both source domains. On Articraft minimax, coupled reaches mean/worst 0.209117/0.225900 versus CR-FPL-minimax 0.192426/0.221139 and equal-parameter independent 0.225819/0.226559, while reducing max-side target-relative p99 to 1.233417 versus 1.447152/1.262213. On NJC minimax, coupled improves object-macro mean/worst to 0.269822/0.269826 versus CR-FPL-minimax 0.295630/0.295631 and independent 0.303314/0.303314, but remains worse than frozen CR-FPL original 0.245383 and has p99 2.145510 versus independent 1.593402. The minimax loss clips gradients on 97.6-99.8% of steps, so its optimization geometry, not insufficient loop contraction, is the limiting factor.

**Result**: PARTIAL/PRUNE_AS_PRIMARY: fixed 3-architecture x 2-loss x 2-domain matrix completed at commit d5938d094755a2fb229833f5b57d9ad812a404b3. Structural gate fails because coupled-minimax does not beat CR-FPL-minimax in Articraft mean/worst and does not beat frozen CR-FPL-original NJC macro mean. Positive evidence: exact swap error <=7.99e-15; every coupled loop prediction residual decreases over four iterations; coupled beats equal-parameter independent on mean/worst in both domains and improves several tails. Next branch should preserve the coupled block but replace the saturating hard minimax training objective with a scale-adaptive constrained allocation/update rule, without weight/seed tuning.

**Branch**: d5938d094755a2fb229833f5b57d9ad812a404b3
