...
Conflicts: Unlike 8.7/8.8 donor-distance matching and 8.9 linear OLS residualization, RQ-LSOT changes the conditional nuisance model rather than relaxing feasibility thresholds; it remains a deterministic falsification ablation rather than a permutation p-value, and if label-free feasibility fails the node is pruned without source labels, training, Box, degree search, or threshold adjustment. [RUNNING]
