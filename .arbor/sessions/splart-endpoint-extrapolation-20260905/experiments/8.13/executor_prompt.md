...
Shared conditional bijection at recipient u: pool all train objects with weight1. Store each training a_j=G_domain(t_j) and u_j. Require pooled a unique. h_N=(1/sqrt(3))*(4/(3N))^(1/5), no selection. For query u, w_j=exp(-(u-u_j)^2/(2h_N^2)); n_eff=(sumw)^2/sumw^2>=2. Sort by a; conditional mid-CDF knots p_j(u)=(0.5 + sum_{k<j}w_k + 0.5*w_j)/(1+sumw); eta knots=logit(p_j). They are strict; define H_u:a->eta by piecewise linear internal + positive linear endpoint tails, exact inverse eta->a. Forward eta=H_u(G_d(t)); update r+=(eta-t)v. Train sequentially on updated r. Inverse for recipient uses same stored slice model in exact reverse order: eta=vTr, a=H_u^{-1}(eta), t=G_d^{-1}(a), update r+=(t-eta)v.
