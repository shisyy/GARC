...
Fixed first stage per domain: object means y, raw z; own train ECDF u=(x+1)/2. Positive-QR OLS A_d=[1,z_centered]B_d, residual r=(y-A_d)/s_d where s_d=sqrt(mean over all object-by-coordinate residual entries of residual^2); require s_d^2 > rank_relative_tolerance times the centered-field mean square computed over the same object-by-coordinate entries, with no floor. Mechanical d excluded.
