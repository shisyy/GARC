...
**ID**: 8.15
**Hypothesis**:
Mechanism: Anchor-Relative Log-Distance Primal-Dual Loop (ALD-PDL) replaces the additive correction cap with a cumulative log(target-distance / anchor-distance) primal state, while the recurrent dual state damps only increases in a source-train-defined log-energy and the same recalled pre-norm Transformer block remains tied for four loops.
...
Implement ALD-PDL from node 8.14 immediately. Keep exactly 4 tied loops, width 64, seed 2202, unchanged 13/6 source split and 4000 steps. Replace only the additive bounded primal action with a positive anchor-relative log-distance state; no hand-tuned correction cap or threshold. Add parameter-matched one-loop inference control using the same trained parameters, mean-side and worst-side NMAE, target-relative p99 error, exact swap and per-loop diagnostics. Reuse the authorized source cache and never read protected labels. Run focused local tests, commit, then report an exact remote launch command.
...
Save results to `results/8.15-<brief-description>/`.
