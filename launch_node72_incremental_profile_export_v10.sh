...
output="$root/node72-profiles-incremental-1-v10"
test "$(stat -c %a "$root")" = 700
test "$(df --output=avail -B1 /data1 | tail -1)" -ge 34359738368
test "$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits -i 5)" -lt 1024
test -z "$(nvidia-smi --query-compute-apps=pid --format=csv,noheader -i 5)"
test ! -e "$output"
...
/data/yptang/workspace/20260720_SplArt/.conda/splart/bin/python \
  ...
  --training-outputs "$root/node72-incremental-completed-v11.json" \
