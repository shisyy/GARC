#!/usr/bin/env python3
"""Fit two endpoint scalars with OEC-CEA on a frozen SplArt model."""
...
from splart.oec_certificate import oec_certificate
...
def main() -> None:
    ...
    if any(marker in launch_text for marker in ("sealed", "evaluator_truth", "full22", "b_test")):
        raise ValueError("model-facing OEC launch contains evaluator-only path marker")

    prior_path = args.prior_prediction.resolve()
    if sha256_file(prior_path) != PRIOR_PREDICTION_SHA256:
        raise ValueError("OEC must consume the single frozen D2-CEA v2 prediction")
    ...
    fitted = fit_oec_endpoints(renderer.model, renderer.ns_s_raw, iterations=300, lr=3e-2)
    lower, upper = float(fitted.lower.cpu()), float(fitted.upper.cpu())
    certificate = oec_certificate(renderer.model, lower, upper, renderer.ns_s_raw)
    ...
    if before_hash != after_hash or any(parameter.grad is not None for parameter in renderer.model.parameters()):
        raise RuntimeError("OEC changed or differentiated through the frozen SplArt model")
    ...
    prediction["renderer"] = "original-splart-plus-oec-cea-scalars"
    prediction["training"] = dict(prior["training"])
    prediction["training"]["oec_posthoc_only"] = False
    prediction["training"]["oec_cea_iterations"] = 300
    prediction["training"]["base_parameters_frozen"] = True
    prediction["oec"] = {
        "schema": "splart-oec-certificate/v1",
        ...
        "module_sha256": sha256_file(certificate_source / "src" / "splart" / "oec_certificate.py"),
        "runner_sha256": sha256_file(Path(__file__).resolve()),
        "model_state_sha256_before": before_hash,
        "model_state_sha256_after": after_hash,
        "evidence": asdict(certificate),
        "fit": {"initial_lower": -0.5, "initial_upper": 1.5, "final_loss": float(fitted.loss.cpu())},
    }
