#!/usr/bin/env python3
"""Train/evaluate CSTR on the unchanged NJC 11/4 source gate."""
...
from build_njc_full_trajectory_profiles import EXPECTED_SPLIT_EPISODES, EXPECTED_SPLIT_OBJECTS, FORBIDDEN_PATH_MARKERS
from run_cr_fpl_njc_gate import load_njc, validate_njc_index
from run_cr_fpl_source_gate import CONFIG, normalize_source_for_requery
from run_glpdt_source_gate import sha256_file
...
def _load_cr_fpl_reference(path: Path, index_sha256: str) -> dict[str, float]:
    ...
    if payload.get("schema") != "splart-cr-fpl-njc-gate/v1":
        raise ValueError("unexpected CR-FPL NJC report schema")
    if payload.get("source_index", {}).get("sha256") != index_sha256:
        raise ValueError("CR-FPL reference used a different NJC source index")
    metrics = payload.get("metrics", {})
    return {
        "object_macro_mean_side_nmae": float(metrics["cr_fpl_object_macro_endpoint_nmae"]),
        "object_macro_worst_side_nmae": float(metrics["cr_fpl_object_macro_worst_side_nmae"]),
        "target_relative_p99": float(metrics["cr_fpl_target_relative_p99_error"]),
        "object_macro_one_loop_mean_side_nmae": float(metrics["one_loop_same_parameters_object_macro_endpoint_nmae"]),
    }
...
def _joined_data(geometry_index: Path, semantic_index: Path, control: str) -> dict[str, tuple[Any, ...]]:
    ...
    rows = validate_njc_index(geometry_index.resolve())
    expected = {(row["split"], row["object_id"], row["episode_id"]) for row, _ in rows}
    geometry = normalize_source_for_requery(load_njc(geometry_index))
    semantic = load_semantic_cache(
        semantic_index,
        source_index_sha256=index_sha256,
        expected_split_counts=EXPECTED_SPLIT_EPISODES,
    ...
    geometry_episodes: dict[str, list[tuple[str, str]]] = {split: [] for split in EXPECTED_SPLIT_EPISODES}
    for row, _ in rows:
        geometry_episodes[row["split"]].append((row["episode_id"], row["object_id"]))
    joined = {}
    for split, values in geometry.items():
        ordered = sorted(geometry_episodes[split])
        expected_episode_ids = tuple(value[0] for value in ordered)
        expected_object_ids = tuple(value[1] for value in ordered)
        evidence, semantic_coordinates, object_ids, episode_ids = semantic[split]
        if values[4] != expected_object_ids or object_ids != expected_object_ids or episode_ids != expected_episode_ids:
            raise ValueError("NJC semantic trajectories are not aligned to geometry episodes")
...
def run(
    ...
    with torch.no_grad():
        ...
        add_object_macro_metrics(metrics, output, validation[3], validation[4])
        ...
        add_cr_fpl_object_macro_metrics(metrics, baseline_output, validation[3], validation[4])
        ...
        metrics["cstr_residual_endpoint_object_macro_auroc"] = semantic_endpoint_object_macro_auroc(
            validation[5], validation[6], validation[3], validation[4]
        )
        report = {
            "schema": "splart-cstr-njc-gate/v1",
            ...
            "primary_metric": "candidate_object_macro_mean_side_nmae",
            "metrics": metrics,
...
def main() -> None:
    ...
    if any(marker in launch for marker in FORBIDDEN_PATH_MARKERS):
        raise ValueError("CSTR NJC launch contains a protected path marker")
    print(
        json.dumps(
            run(
                args.index,
