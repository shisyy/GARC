#!/usr/bin/env python3
"""Seal geometry-first predictions and routing evidence before independent scoring.
...
from splart.geometry_first import MODES, geometry_first_predict

COUNTS = {"source_train": 13, "source_validation": 6}
CODE_PATHS = ("src/splart/relative_search.py", "src/splart/geometry_first.py",
              "run_geometry_first_gate.py", "evaluate_relative_search.py")
    ...
    for split in COUNTS:
        ...
        for method in METHODS:
            ...
            counts = {key: sum(s.get(key) is True for s in sides) for key in (
                "geometric_confirmed", "role_eligible", "semantic_verified", "semantic_activation", "selection_changed")}
            fallback = Counter()
            for side in sides:
...
def run(inputs: Path, output_dir: Path) -> dict:
    ...
    manifest = {"schema": "geometry-first-all-modes-seal/v1", "modes": list(MODES),
    ...
    for mode in MODES:
        ...
        for value in values:
            ...
            for method in METHODS:
                ...
                prediction = geometry_first_predict(value, method, mode, semantic, role)
                swapped = geometry_first_predict(swap_input(value), method, mode,
                ...
                diagnostics.append({"object_id": value["object_id"], "split": value["split"], "method": method,
                                    ...
                                    "sides": [s.get("geometry_first", {}) for s in prediction["sides"]]})
        sidecar = {"schema": "geometry-first-routing-diagnostics/v1", "mode": mode, **bindings,
        ...
        report = {"schema": "relative-search-predictions/v1", "config": asdict(CONFIG), "methods": list(METHODS),
                  "geometry_first_mode": mode, "preregistered_modes": list(MODES), **bindings,
...
def verify_all_modes(output_dir: Path) -> dict:
    ...
    if (manifest["schema"] != "geometry-first-all-modes-seal/v1" or manifest["modes"] != list(MODES)
    ...
    for mode in MODES:
        ...
        if (payload["geometry_first_mode"] != mode or diagnostics["mode"] != mode
                or payload["preregistered_modes"] != list(MODES)):
            raise ValueError("mode identity mismatch")
        for key in ("source_index_sha256", "implementation_sha256", "input_index_sha256"):
...
if __name__ == "__main__":
    ...
    print(json.dumps(run(args.inputs, args.output_dir), sort_keys=True))
